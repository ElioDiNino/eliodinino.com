...
export const Picture = (props: PictureProps) => {
  ...
  imageSize = picture.isMobile
    ? (imageSize = picture.imageSizeMobile)
    : (imageSize = picture.imageSizeNormal);
  ...
  jpgSrcSet =
    picture.image === 'profile'
      ? (jpgSrcSet = profileJpgSrcSet)
      : picture.image === 'logo'
        ? (jpgSrcSet = logoPngSrcSet)
        : (jpgSrcSet = '');
  webpSrcSet =
    picture.image === 'profile'
      ? (webpSrcSet = profileWebpSrcSet)
      : picture.image === 'logo'
        ? (webpSrcSet = logoWebpSrcSet)
        : (webpSrcSet = '');
  avifSrcSet =
    picture.image === 'profile'
      ? (avifSrcSet = profileAvifSrcSet)
      : picture.image === 'logo'
        ? (avifSrcSet = logoAvifSrcSet)
        : (avifSrcSet = '');

  defaultImage =
    picture.image === 'profile'
      ? (defaultImage = profile799jpg)
      : picture.image === 'logo'
        ? (defaultImage = logopng)
        : (defaultImage = '');

  return (
    <picture>
      <source type="image/avif" sizes={imageSize} srcSet={avifSrcSet} />
      <source type="image/webp" sizes={imageSize} srcSet={webpSrcSet} />
      <img
